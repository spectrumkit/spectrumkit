...
`@spectrumkit/spectrumkit-siwe-next-auth` paketini kurun.
...
Kullanıcı başarılı bir şekilde kimlik doğrulaması yaptığında, oturum token'ının `sub` özelliği (token'ın "konusu", yani kullanıcı) kullanıcının adresi olacaktır.
...
- [Next.js kimlik doğrulama rehberi](https://nextjs.org/docs/pages/guides/authentication)
