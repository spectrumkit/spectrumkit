...
```bash
npm i @spectrumkit/spectrumkit@3 wagmi@3 viem@2 @tanstack/react-query@latest
```

**2. Check for breaking changes in `wagmi`**

If you use `wagmi` hooks and `viem` actions in your dApp, you will need to follow the migration guide for v3:
...
RainbowKit now bundles all wallet connector dependencies directly. This provides better tree-shaking support for smaller bundle sizes. You no longer need to install connector packages separately.

**2. No Changes to RainbowKit API**

The RainbowKit API itself remains unchanged. Your existing configuration with `getDefaultConfig` or custom wallet lists will continue to work without modification.
