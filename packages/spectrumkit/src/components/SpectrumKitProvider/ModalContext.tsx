...
import { useConnection, useConnectionEffect, useConfig } from 'wagmi';
...
const ModalContext = createContext<ModalContextValue>(DEFAULT_MODAL_CONTEXT);
...
export function ModalProvider({ children }: ModalProviderProps) {
  // ModalProvider calls wagmi hooks (useConnection, useConfig, ...) which
  // throw WagmiProviderNotFoundError on SSR. Bail to default-context render
  // on the server and first hydration pass, then swap in the real provider
  // on client mount.
  const mounted = useIsMounted();
  if (!mounted) {
    return (
      <ModalContext.Provider value={DEFAULT_MODAL_CONTEXT}>
        {children}
...
function ModalProviderClient({ children }: ModalProviderProps) {
  const {
