...
export function SpectrumKitProvider({
  ...
  const themedChildren = theme ? (
    <div {...createThemeRootProps(id)}>
      <style
        // Injected content is developer-supplied theme config (not user
        // input) and is sanitized: createThemeRootSelector gates the id to
        // /^[a-zA-Z0-9_]+$/, and cssStringFromTheme strips :;{}</> from every
        // value, preventing value/`</style>` breakout.
        // biome-ignore lint/security/noDangerouslySetInnerHtml: sanitized theme CSS, not user input (see above)
