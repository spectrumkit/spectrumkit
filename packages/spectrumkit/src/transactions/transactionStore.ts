...
const storageKey = 'sk-transactions';
...
// Stable reference returned when no transactions exist for an account/chain.
// Required because useSyncExternalStore re-renders when getSnapshot returns
// a new reference; a fresh `[]` literal would create an infinite render loop.
...
export function createTransactionStore({
  ...
  function getTransactions(account: string, chainId: number): Transaction[] {
    return data[account]?.[chainId] ?? EMPTY_TRANSACTIONS;
  }
