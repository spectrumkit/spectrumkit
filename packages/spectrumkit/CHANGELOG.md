...
- Shrink the published package and remove dead code.

  - Prune 45 unreferenced locale keys (the `intro.*` / `get.*` panel strings
    orphaned when `ConnectModalIntro` was removed) across all 19 locale files, and
    minify the embedded locale JSON at build time — together ~26% off the locale
    payload, with a smaller eagerly-loaded `en_US` chunk.
  - Rasterize three oversized wallet icons that shipped as multi-megapixel PNGs
    wrapped in an SVG shell (`frameWallet` was a 2558×778 image for a 28–60px
    icon), cutting those icon chunks ~95%.
  - Delete unreachable code: the orphaned `InfoButton`/`Info` cluster, four unused
    preload icons and their assets, 11 browser-icon SVGs with no importer, the
    duplicate `components/index.ts` barrel, the dead `getGradientRGBAs` /
    `convertHexToRGBA` color helpers, and a vestigial `t` on the i18n context.
