...
type InstructionVariant = 'mobile' | 'extension' | 'desktop';

type Instructions = NonNullable<
  NonNullable<WalletConnector['qrCode']>['instructions']
>;

const INSTRUCTION_VARIANT_CONFIG: Record<
  InstructionVariant,
  {
    source: (w: WalletConnector) => Instructions | undefined;
    action: 'connect' | 'refresh';
  }
> = {
  mobile: { source: (w) => w.qrCode?.instructions, action: 'connect' },
  extension: { source: (w) => w.extension?.instructions, action: 'refresh' },
  desktop: { source: (w) => w.desktop?.instructions, action: 'connect' },
};
...
function InstructionDetail({
  ...
  const { i18n } = useContext(I18nContext);
  const { source, action } = INSTRUCTION_VARIANT_CONFIG[variant];
  const instructions = source(wallet);
  ...
  return (
    ...
    >
      ...
      >
        <ActionButton
          label={i18n.t(`get_instructions.${variant}.${action}.label`)}
        ...
        >
          <Text color="accentColor" size="14" weight="bold">
            {i18n.t(`get_instructions.${variant}.learn_more.label`)}
          </Text>
        </Box>
      </Box>
    </Box>
...
export function InstructionMobileDetail(props: {
  connectWallet: (wallet: WalletConnector) => void;
  wallet: WalletConnector;
}) {
  return <InstructionDetail variant="mobile" {...props} />;
}
...
export function InstructionExtensionDetail({
  ...
  return <InstructionDetail variant="extension" wallet={wallet} />;
}

export function InstructionDesktopDetail(props: {
  connectWallet: (wallet: WalletConnector) => void;
  wallet: WalletConnector;
}) {
  return <InstructionDetail variant="desktop" {...props} />;
}
