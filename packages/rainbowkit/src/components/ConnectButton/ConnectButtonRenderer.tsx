...
export function ConnectButtonRenderer(props: ConnectButtonRendererProps) {
  // wagmi Config context isn't available on SSR. Defer the whole renderer
  // until client mount so `useConnection` / `useConfig` don't throw.
  const mountedSync = useIsMountedSync();
  if (!mountedSync) return null;
  return <ConnectButtonRendererClient {...props} />;
}

function ConnectButtonRendererClient({ children }: ConnectButtonRendererProps) {
